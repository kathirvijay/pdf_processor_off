    ...
    sortedBoxes.forEach((box) => {
      try {
        ...
        if (content && content.trim()) {
          ...
          try {
            doc.font(safeFontFamily).fontSize(fitFontSize);
            let needH = doc.heightOfString(content, { width: textWidthSafe });
            while (needH + lineGapPt > textHeight && fitFontSize > minFontSize) {
              fitFontSize = Math.max(minFontSize, Math.floor(fitFontSize * 0.85));
              doc.fontSize(fitFontSize);
              needH = doc.heightOfString(content, { width: textWidthSafe });
            }
          } catch (_) { fitFontSize = fontSize; }
          doc.font(safeFontFamily).fontSize(fitFontSize).fillColor(r, g, b);
          doc.text(content, textX, textY, {
            width: textWidthSafe,
            height: textHeight,
            align: box.properties?.alignment || 'left',
            lineGap: lineGapPt,
            ellipsis: false
          });
          doc.restore();
